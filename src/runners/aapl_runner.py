...
from src.strategies.aapl_3attempt import AaplStrategy, Phase, SYMBOL
from src.config import LOGS_DIR

load_dotenv()

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("webull.aapl_runner")
...
def log_summary(strategy):
    path = LOGS_DIR / "aapl_3attempt.jsonl"
...
def run(client):
    ...
    log.info("AAPL 3-Attempt strategy started. Waiting for 9:40 AM EST...")
    try:
